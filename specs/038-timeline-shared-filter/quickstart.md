...
| 3 | Type `map`, then `session:S1`, then `"a phrase"`, then `session:`. | Rows that only say `nmap` are not lit, as in Search. The read-out shows condition and text tokens. `session:` reads as unparsable. Non-matches are dimmed, not removed. | US3, SC-003 |
