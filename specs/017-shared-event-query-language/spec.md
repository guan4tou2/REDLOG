...
- An identifier condition that matches nothing is distinct from one that could
  not be parsed.
- A term that could only match output held in a terminal recording is reported
  as beyond the query's coverage rather than as no match.
...
- **FR-006**: A recognised prefix MUST also be expressible as literal text.
  "Which records quote this ID" is a real question — it is how an operator finds
  where an identifier was pasted — and it is the only way to ask it once the
  prefix becomes a condition.
- **FR-007**: Whether a field name is recognised MUST NOT depend on its case,
  because the text path folds case and an operator who types a field in capitals
  is not asking a different question.
- **FR-008**: Each surface MUST show which parts of the query were read as
  conditions and which as text.
- **FR-009**: Structured conditions and free text in one query MUST all apply
...
- **FR-010**: Free text MUST keep the matching behaviour the store already
  provides: whole-token phrases, so punctuation-bearing text such as a URL, an
  IP address or a `-sV` flag matches as typed, and a prefix-matched final term,
  so a partially typed word still matches while it is being typed.
- **FR-011**: A query MUST preserve the Transcript's per-bucket balance and MUST
  expose `hasMore` and a cursor per bucket exactly as the unqueried view does.
- **FR-012**: When matched tool calls or results have counterparts outside the
  loaded set, those counterparts MUST be retrieved for the page as a whole
  rather than one lookup per unpaired record.
- **FR-013**: A pair whose counterpart cannot be retrieved MUST be shown as
  explicitly incomplete; one half MUST NOT be presented as a whole exchange.
- **FR-014**: Every displayed result MUST keep the source occurrence time and
  the RedLog receipt time distinguishable.
- **FR-015**: Query failure, no match and unparsable query MUST remain distinct
  states, each carrying its recovery action. There is no not-yet-indexed state
  here: the Transcript searches stored event content, and no pending-index
  signal exists for it. Search's cast-index state is Spec 018's to preserve.
- **FR-016**: Clearing the query MUST restore the unqueried first page without
  rows from the query result set.
- **FR-017**: The completeness indicator MUST describe the dataset the current
  query actually covers.
- **FR-018**: A filter that still operates only over loaded evidence MUST say
  so where it is offered.
- **FR-019**: The Transcript MUST disclose that a text query covers stored
  event content only. Command output that was not captured inline — the
  external-shell hook records metadata, with the bytes in a terminal recording
  — is outside what a term can match, and an unmatched term MUST NOT be read
  as proof that the output never contained it.
...
- **SC-010**: A term present only in command output that was not captured
  inline is reported as outside the query's reach, not as absent.
