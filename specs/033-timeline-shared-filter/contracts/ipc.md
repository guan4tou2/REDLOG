...
take the active project's policy from main, not from the renderer. With no
active project, each one returns its empty shape. None of them catches a query
error: a failure reaches the renderer as a rejected promise (Constitution VI).
...
(req: {
  ...
  filter?: EventFilter
  cursor?: string | null         // count strictly past this position
