...
//
// Expectations that differ from the capture on purpose:
//   - the built-in CTF flag pattern is removed (product identity: no
//     proof/flag/exam), so the line that used to yield `flag` matches yields
//     none;
//   - Spec 031: the private-key value is the header plus its first line of key
//     material, not the algorithm name (`RSA `) the old `m[1] || m[0]` took —
//     that made every RSA key the same value.
...
const EXPECTED_LOOT: Array<Array<[string, string, string]>> = [[["generic_api_key", "abcd1234efgh5678", "medium"]], [["generic_api_key", "hunter2hunter2", "medium"]], [], [["aws_key", cat("AK", "IAIOSFODNN7EXAMPLE"), "high"]], [], [["private_key", cat("-----BEGIN RSA PRIV", "ATE KEY-----\nMIIEpAIBAAKCAQEA"), "high"]], [["jwt", JWT, "medium"]], [], [], [], [], [], [], [], [["database_url", PG_URI, "high"]], [["database_url", "redis://cache.local:6379", "high"]], [], [["password_hash", "$6$saltsalt$abcdefghijklmnopqrstuvwxyz./ABCD", "high"]], [["ntlm_hash", "aad3b435b51404eeaad3b435b51404ee:31d6cfe0d16ae931b73c59d7e0c089c0", "high"]], [["password_hash", "$6$xyz$hashedvalue", "high"], ["shadow_entry", "root:$6$xyz$hashedvalue:19000:0:99999:7:", "high"]], [["base64_creds", BASIC_LINE, "medium"]], [], [], [["generic_api_key", "longsecretvalue123", "medium"]]]
