...
describe('classifyIP — the A-1..A-9 combination matrix', () => {
  ...
  const cases: Array<[string, string, string[], string[], 'safe' | 'exposed' | 'unknown']> = [
    ['A-1  no lists at all                    ', NEITHER, [], [], 'unknown'],
    ['A-2  blacklist only, hit                ', HOME, [], BL, 'exposed'],
    ['A-3  blacklist only, miss               ', NEITHER, [], BL, 'safe'],
    ['A-4  whitelist only, hit                ', VPN, WL, [], 'safe'],
    ['A-5  whitelist only, miss               ', NEITHER, WL, [], 'unknown'],
    ...
    ['A-9  both, hits NEITHER                 ', NEITHER, WL, BL, 'unknown']
  ]
...
describe('IPMonitor honours the fixed classification end-to-end', () => {
  ...
  it('reports a whitelist miss as unknown, not safe, when a blacklist is also configured', async () => {
