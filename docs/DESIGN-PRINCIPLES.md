...
> **§1 was superseded on 2026-08-22.** This document was written before the
> core revision recorded in [`DESIGN-core-and-capture.md`](DESIGN-core-and-capture.md),
> which moved the irreducible core from *defensibility* (a tamper-evident
> record a third party can verify) to *knowability* (nothing missing, findable
> afterwards — the operator reconstructing and writing up their own engagement
> is the primary consumer). Tamper-evidence is kept, but it is no longer what
> the product is for, and §1's test below reads a feature's worth by the wrong
> yardstick.
>
> Where §1 and the core note disagree, the core note wins — which inverts the
> "the principle wins and the doc is the bug" rule two paragraphs down, for §1
> and only §1. Every other principle here still holds and several are cited by
> shipped code.
>
> Salvaged from PR #8 unmerged rather than rewritten: the reasoning is worth
> reading in the form it was argued, and rewriting history to agree with a
> later decision hides that the decision was made.
