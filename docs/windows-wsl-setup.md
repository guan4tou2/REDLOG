...
(config: `electron-builder.yml`). Native deps are rebuilt for the target
Electron automatically during packaging.

```powershell
npm run dist:win       # NSIS installer -> dist\RedLog-<version>-win-x64.exe
npm run pack:dir       # unpacked dir only (fast smoke test)
```
