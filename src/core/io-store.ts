...
/** Resolve a ref to its on-disk path iff it is a well-formed sha256 that
 *  resolves inside the project's io/ dir. Returns null for anything else —
 *  this is the path-traversal guard shared by read and verify. */
export function resolveRef(projectDir: string, ref: string): string | null {
  ...
  const resolved = path.resolve(dir, `${ref}.bin`)
...
export function putBody(projectDir: string, bytes: Buffer): IoRef {
  ...
  // same bytes — never rewrite it (keeps the store append-only + cheap).
...
export function readBody(projectDir: string, ref: string, off?: number, len?: number): Buffer | null {
  const file = resolveRef(projectDir, ref)
  if (!file) return null
  let stat: fs.Stats
  try { stat = fs.statSync(file) } catch { return null }   // pruned / never written
  ...
  // Range read (mirrors readCastRange guards).
  const start = off ?? 0
...
/** True iff the ref is well-formed and its on-disk bytes hash to the ref.
 *  Used by verify: a mismatch is tampering, an absent file is pruning (caller
 *  checks existence separately via readBody === null with a valid ref). */
